...
namespace potential_gap {
    ...
    bool GoalSelector::setGoal(
        const std::vector<geometry_msgs::PoseStamped> &plan, geometry_msgs::TransformStamped rbt2map) {
        // Incoming plan is in map frame
        boost::mutex::scoped_lock lock(goal_select_mutex);
        boost::mutex::scoped_lock gplock(gplan_mutex);
        global_plan.clear();
        global_plan = plan;
        // transform plan to map frame
        for (int i = 0; i < global_plan.size(); i++) {
            tf2::doTransform(global_plan.at(i), global_plan.at(i), rbt2map);
        }
    ...
    void GoalSelector::updateLocalGoal() {
        if (global_plan.size() < 2) {
            // No Global Goal
            return;
        }
        ...
        boost::mutex::scoped_lock glock(goal_select_mutex);
        boost::mutex::scoped_lock llock(lscan_mutex);
        auto local_gplan = getRelevantGlobalPlan();
        if (local_gplan.size() < 1) return;
        ...
        if (cfg_->planning.far_feasible) {
            ...
            // ROS_INFO_STREAM("Cloestest: " << result_fwd - local_gplan.begin());
            // local_goal = *std::prev(result_fwd);
            result_fwd = result_fwd == local_gplan.end() ? result_fwd - 1 : result_fwd;
            local_goal = local_gplan.at(result_fwd - local_gplan.begin());
        }
    }
    ...
    std::vector<geometry_msgs::PoseStamped> GoalSelector::getRelevantGlobalPlan() {
        // Global Plan is now in robot frame
        // Do magic with egocircle
        boost::mutex::scoped_lock gplock(gplan_mutex);
        mod_plan.clear();
    ...
    geometry_msgs::PoseStamped GoalSelector::sensorFrameLocalGoal(geometry_msgs::TransformStamped rbt2cam) {
        geometry_msgs::PoseStamped result;
        tf2::doTransform(local_goal, result, rbt2cam);
        // This should return something in odom frame
        return result;
    }

    std::vector<geometry_msgs::PoseStamped> GoalSelector::getGlobalPlan() {
        return global_plan;
    }

    std::vector<geometry_msgs::PoseStamped> GoalSelector::getOdomGlobalPlan(geometry_msgs::TransformStamped map2odom)
    {
        std::vector<geometry_msgs::PoseStamped> plan;
        for (int i = 0; i < global_plan.size(); i++) {
            geometry_msgs::PoseStamped plan_pose;
            tf2::doTransform(global_plan.at(i), plan_pose, map2odom);
