...
namespace potential_gap {
    TrajectoryArbiter::TrajectoryArbiter(ros::NodeHandle& nh, const potential_gap::PotentialGapConfig& cfg, RobotGeoProc& robot_geo_proc)
    {
        ...
        orient_weight = cfg_->traj.orient_weight;
    ...
    std::vector<double> TrajectoryArbiter::scoreTrajectory(geometry_msgs::PoseArray traj) {
        ...
        // ROS_INFO_STREAM("Raw total score: " << total_val);
        ...
        if (cost_val.size() > 0) // && ! cost_val.at(0) == -std::numeric_limits<double>::infinity())
        {
            ...
            {
                auto orient_cost = orient_weight * orientCost(*std::prev(traj.poses.end()));
                cost_val.at(0) -= orient_cost;
            }
        }
    ...
    double TrajectoryArbiter::orientCost(geometry_msgs::Pose pose)
    {
        boost::mutex::scoped_lock planlock(gplan_mutex);

        double pose_ang = atan2(pose.position.y, pose.position.x);
        
        return abs(pose_ang) / M_PI;
    }
    ...
    double TrajectoryArbiter::scorePose(geometry_msgs::Pose pose) {
        ...
        for (int i = 0; i < dist.size(); i++) {
            ...
            double nearest_dist;
            if(robot_geo_proc_.robot_.shape == RobotShape::box)
            {
                if(robot_geo_proc_.robot_.simple_boundary)
                    nearest_dist = rel_pt_vec.norm() - robot_geo_proc_.getRobotMaxRadius() * cfg_->traj.inf_ratio;
                else
                    nearest_dist = robot_geo_proc_.getNearestDistance(orient_vec, rel_pt_vec);
            }
            else if (robot_geo_proc_.robot_.shape == RobotShape::circle)
            {
                nearest_dist = rel_pt_vec.norm() - robot_geo_proc_.getRobotMaxRadius() * cfg_->traj.inf_ratio;
            }
