...
namespace potential_gap{
    ...
    void GapVisualizer::drawGap(visualization_msgs::MarkerArray & vis_arr, potential_gap::Gap g, std::string ns, std::string color) {
        ...
        if (color_value == colormap.end()) {
            ROS_WARN_STREAM("Visualization Color not found, return without drawing");
            return;
        }
    ...
    void GapVisualizer::drawManipGap(visualization_msgs::MarkerArray & vis_arr, potential_gap::Gap g, bool & circle) {
        ...
        if (!g.mode.reduced && !g.mode.convex && !g.mode.rgc) {
            return;
        }
        ...
        // if (g.mode.convex && (g.convex.convex_ridx < 256 || circle)) {
        //     return;
        // }
        ...
        if (g.mode.convex) {
            ...
            if (!circle) {
                for (int i = 0; i < 50; i++) {
                    ...
                    this_marker.id = id++;
                    this_marker.lifetime = ros::Duration(1./5);
                    vis_arr.markers.push_back(this_marker);
                }
                circle = true;
            }
    ...
    // void GapVisualizer::drawGapGoal(visualization_msgs::MarkerArray & vis_arr, const potential_gap::Gap g) {
    //     if (!g.goal.set) {
    //         return;
    //     }
    ...
    // void GapVisualizer::drawGapGoals(const std::vector<potential_gap::Gap> vec) {
    //     visualization_msgs::MarkerArray vis_arr;
    //     for (auto & gap : vec) {
    //         drawGapGoal(vis_arr, gap);
    //     }
    //     gapgoal_publisher.publish(vis_arr);
    //     return;
    // }
