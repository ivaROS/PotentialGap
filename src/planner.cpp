...
namespace potential_gap
{
    ...
    bool Planner::initialize(const ros::NodeHandle& unh)
    {
        ...
        double length = 0.7, width = 0.3, decay_factor = 0, avg_lin_speed = 0.3, avg_rot_speed = 0.5;
        ...
        bool robot_holonomic = true, simple_boundary = false;
        unh.getParam("robot_holonomic", robot_holonomic);
        unh.getParam("simple_boundary", simple_boundary);
        unh.setParam("robot_holonomic", robot_holonomic);
        unh.setParam("simple_boundary", simple_boundary);

        RobotShape robot_shape = static_cast<RobotShape>(shape_id);
        if(robot_shape == RobotShape::circle)
            width = 0;

        Robot robot(robot_shape, length, width, avg_lin_speed, avg_rot_speed, robot_holonomic, simple_boundary);
        ...
        unh.getParam("pf_local_frame_enable", pf_local_frame_enable_);
        unh.setParam("pf_local_frame_enable", pf_local_frame_enable_);
    ...
    void Planner::configCB(potential_gap::CollisionCheckerConfig &config, uint32_t level)
    {
        ...
        std::shared_ptr tf_buffer_tmp = std::make_shared<tf2_ros::Buffer>();
        std::shared_ptr tf_listener_tmp = std::make_shared<tf2_ros::TransformListener>(*tf_buffer_tmp);
        if(config.cc_type != cc_type_)
        {
            if(config.cc_type == potential_gap::CollisionChecker_depth)
            {
                ROS_INFO_STREAM("New cc type = depth");
                cc_wrapper_ = std::make_shared<pips_trajectory_testing::DepthImageCCWrapper>(nh, pnh, tf2_utils::TransformManager(tf_buffer_tmp, tf_listener_tmp));
            }
            else if(config.cc_type == potential_gap::CollisionChecker_depth_ego)
            {
                ROS_INFO_STREAM("New cc type = depth ego");
                cc_wrapper_ = std::make_shared<pips_egocylindrical::EgocylindricalRangeImageCCWrapper>(nh, pnh, tf2_utils::TransformManager(tf_buffer_tmp, tf_listener_tmp));
            }
            else if(config.cc_type == potential_gap::CollisionChecker_egocircle)
            {
                ROS_INFO_STREAM("New cc type = egocircle");
                cc_wrapper_ = std::make_shared<pips_egocircle::EgoCircleCCWrapper>(nh, pnh, tf2_utils::TransformManager(tf_buffer_tmp, tf_listener_tmp));
            }
    ...
    void Planner::laserScanCB(boost::shared_ptr<sensor_msgs::LaserScan const> msg)
    {
        ...
        try {
            ...
            {
                g.transform(trans, trans_min_dist);
            }
            gapvisualizer->drawGaps(observed_gaps, std::string("trans"));
    ...
    bool Planner::setGoal(const std::vector<geometry_msgs::PoseStamped> &plan, bool is_global_plan)
    {
        updateLocalTF();
        updateGlobalTF();
	...
	// plan should be in global frame
        ...
        if(!is_global_plan)
        {
            tf2::doTransform(final_goal_odom, final_goal_odom, rbt2odom);
        }
        else
        {
        ...
        if(!is_global_plan)
        {
            goalselector->setGoal(global_plan, rbt2map);
        }
        else
        {
        ...
        trajvisualizer->rawGlobalPlan(goalselector->getGlobalPlan());
        ...
        goalvisualizer->localGoal(new_local_waypoint);
        ...
        { 
            ...
            // auto traj = goalselector->getRelevantGlobalPlan();
...
//         ROS_INFO_STREAM("observed_gaps count:" << observed_gaps.size());
    ...
    std::vector<std::vector<double>> Planner::initialTrajGen(std::vector<potential_gap::Gap> vec, std::vector<geometry_msgs::PoseArray>& res, std::vector<geometry_msgs::PoseArray>& virtual_decayed) {
		...
		// Generate trajectory in robot frame.
                ...
                if(!pf_local_frame_enable_)
                {
                    ...
                    // ret_traj.at(i) = gapTrajSyn->transformBackTrajectory(tmp, cam2odom, cfg.sensor_frame_id, cfg.odom_frame_id);
                    ret_traj.at(i) = gapTrajSyn->transformBackTrajectory(tmp, rbt2odom, cfg.robot_frame_id, cfg.odom_frame_id);
                }
                else
                {
		            auto virtual_score_path = getOrientDecayedPath(tmp);
                    virtual_traj.at(i) = virtual_score_path;
                    ret_traj_scores.at(i) = trajArbiter->scoreTrajectory(virtual_score_path);
                    // ret_traj.at(i) = gapTrajSyn->transformBackTrajectory(tmp, cam2rbt, cfg.sensor_frame_id, cfg.robot_frame_id);
                }
            }
        } catch (...) {
    ...
    geometry_msgs::PoseArray Planner::pickTraj(std::vector<geometry_msgs::PoseArray> prr, std::vector<std::vector<double>> score, std::vector<potential_gap::Gap> gaps, std::vector<geometry_msgs::PoseArray> virtual_path, geometry_msgs::PoseArray& chosen_virtual_path) {
        ...
        if (prr.size() == 0) {
            ...
            potential_gap::StaticGap empty_gap;
            potential_gap::StaticInfGap empty_inf_gap(rbt2odom);
            current_gap_ = empty_gap;
            current_inf_gap_ = empty_inf_gap;
        ...
        try {
            // DURATION_INFO_STREAM("Pick Traj omp", 1);
            ...
            for (size_t i = 0; i < result_score.size(); i++) {
                ...
                ROS_INFO_STREAM_COND(print_debug_info_, "Score: " << result_score.at(i));
            }
        } catch (...) {
        ...
        potential_gap::StaticGap current(gaps.at(idx), rbt2map); // Note: using rbt2map is compatible with a unused mpc planner. Not changing it to rbt2odom for now.
        current_gap_ = current;
        if((robot_geo_proc_.robot_.shape == RobotShape::box && robot_geo_proc_.robot_.holonomic) || (!cfg.planning.use_bezier && !cfg.gap_manip.inflated_gap))
        {
            current_inf_gap_ = potential_gap::StaticInfGap(rbt2odom);
        }
        else
        {
    ...
    geometry_msgs::PoseArray Planner::compareToOldTraj(geometry_msgs::PoseArray incoming, geometry_msgs::PoseArray& virtual_curr_traj) {
        ...
        if(incoming.header.frame_id == cfg.robot_frame_id)
        {
            ROS_ERROR("[compareToOldTraj] function does not support robot frame path.");
            throw;
        }
            

        try {
            geometry_msgs::PoseArray incom_rbt;

            // if(!pf_local_frame_enable_)
            {
                incom_rbt = gapTrajSyn->transformBackTrajectory(incoming, odom2rbt, cfg.odom_frame_id, cfg.robot_frame_id);
            }
            // else
            // {
            //     geometry_msgs::TransformStamped rbt2rbt = tfBuffer.lookupTransform(cfg.robot_frame_id, ros::Time::now(), incoming.header.frame_id, incoming.header.stamp, cfg.map_frame_id);
            //     incom_rbt = gapTrajSyn->transformBackTrajectory(incoming, rbt2rbt, cfg.robot_frame_id, cfg.robot_frame_id);
            // }
            ...
            if (curr_traj.poses.size() == 0) {
                if (incom_subscore == -std::numeric_limits<double>::infinity()) {
                    ...
                    potential_gap::StaticGap empty_gap;
                    potential_gap::StaticInfGap empty_inf_gap(rbt2odom);
                    chosen_gap_ = empty_gap;
                    chosen_inf_gap_ = empty_inf_gap;
                    gen_new_plan_ = true;
                    ...
                    setCurrentTraj(incoming);
                    virtual_curr_traj = gapTrajSyn->transformBackTrajectory(virtual_score_path, rbt2odom, cfg.robot_frame_id, cfg.odom_frame_id);
                    trajectory_pub.publish(incoming);
            ...
            geometry_msgs::PoseArray curr_rbt;

            // if(!pf_local_frame_enable_)
            {
                curr_rbt = gapTrajSyn->transformBackTrajectory(curr_traj, odom2rbt, cfg.odom_frame_id, cfg.robot_frame_id);
            }
            // else
            // {
            //     geometry_msgs::TransformStamped rbt2rbt = tfBuffer.lookupTransform(cfg.robot_frame_id, ros::Time::now(), curr_traj.header.frame_id, curr_traj.header.stamp, cfg.map_frame_id);
            //     curr_rbt = gapTrajSyn->transformBackTrajectory(curr_traj, rbt2rbt, cfg.robot_frame_id, cfg.robot_frame_id);
            // }
            ...
            ROS_INFO_STREAM_COND(print_debug_info_, "Curr Score: " << curr_subscore << ", incom Score:" << incom_subscore);
            ...
            double comp_score = curr_subscore;
            // if(!cfg.planning.use_bezier)
            //     comp_score += counts;

            if (incom_subscore > comp_score) {
                ROS_INFO_STREAM("Swap to new for better score: " << incom_subscore << " > " << curr_subscore);
                setCurrentTraj(incoming);
                virtual_curr_traj = gapTrajSyn->transformBackTrajectory(virtual_score_path, rbt2odom, cfg.robot_frame_id, cfg.odom_frame_id);
                trajectory_pub.publish(incoming);
                chosen_gap_ = current_gap_;
            ...
            virtual_curr_traj = gapTrajSyn->transformBackTrajectory(virtual_score_path_curr, rbt2odom, cfg.robot_frame_id, cfg.odom_frame_id);
            trajectory_pub.publish(curr_traj);
            // WARNING: update inflated gap region everytime
            chosen_gap_ = current_gap_;
            chosen_inf_gap_ = current_inf_gap_;
        } catch (...) {
    ...
    CollisionResults Planner::checkCollision(const geometry_msgs::PoseArray path)
    {
        ...
        auto path_rbt = gapTrajSyn->transformBackTrajectory(path, odom2rbt, cfg.odom_frame_id, cfg.robot_frame_id);
    ...
    bool Planner::reachedTrajEnd()
    {
        ...
        auto curr_rbt = gapTrajSyn->transformBackTrajectory(curr_traj, odom2rbt, cfg.odom_frame_id, cfg.robot_frame_id);
        curr_rbt.header.frame_id = cfg.robot_frame_id;
    ...
    std::pair<double, double> Planner::getEgoMinDistArray()
    {
        ...
        if (stored_scan_msgs.ranges.size() < 500) {
            ROS_FATAL_STREAM("[getEgoMinDistArray] Scan range incorrect.");
        }

        std::vector<double> min_dist_arr(stored_scan_msgs.ranges.size());
        for (int i = 0; i < min_dist_arr.size(); i++) {
            float angle = i * stored_scan_msgs.angle_increment - M_PI;
            float dist = stored_scan_msgs.ranges.at(i);
    ...
    geometry_msgs::PoseArray Planner::getPlanTrajectory() {
        ...
        auto picked_traj = pickTraj(traj_set, score_set, gap_set, virtual_traj_set, chosen_virtual_traj_set);
    ...
    geometry_msgs::PoseArray Planner::getSinglePath()
    {
        ...
        ros::WallTime manip_time = ros::WallTime::now();
        ROS_INFO_STREAM("Gap manip time: " << float((manip_time - start).toNSec()) / 1000000 << " ms.");
        ...
        ROS_INFO_STREAM("Gap based init trajs time: " << float((init_time - manip_time).toNSec()) / 1000000 << " ms.");
    ...
    {
        return gapTrajSyn->transformBackTrajectory(input_path, odom2rbt, cfg.odom_frame_id, cfg.robot_frame_id);
    }
