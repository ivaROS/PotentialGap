...
% F= getframe(f);
% img = F.cdata;
% imwrite(img, 'gapfield_empty.jpg');

% 
% ob1 = images.roi.Point(gca,'Position', obs_obj_1(1:2));
% addlistener(ob1,'ROIMoved',@allevents_1);
% ob2 = images.roi.Point(gca,'Position', obs_obj_2(1:2));
% addlistener(ob2,'ROIMoved',@allevents_2);
% 
...
%     obs_2_matrix = ones(1, 2);
    ...
    goal_weight = 1e0;
    ddTraj.minst.Optimization.CustomCostFcn = @(X,U,e,data,params) sum(X(2:end, 1:2) * normal_1' ./ (sqrt(sum(X(2:end, 1:2).^2, 2)) + 1e-8) .* exp(-sqrt(sum((X(2:end, 1:2) - obs_1_matrix).^2, 2)))) +...
                                                                   sum(X(2:end, 1:2) * normal_2' ./ (sqrt(sum(X(2:end, 1:2).^2, 2)) + 1e-8) .* exp(-sqrt(sum((X(2:end, 1:2) - obs_2_matrix).^2, 2)))) +... 
                                                                   goal_weight * sum(sqrt(sum((X(2:end, 1:2) - goal_matrix).^2, 2))) - 10 * sum((sqrt(sum(X(2:end, 1:2).^2, 2)) ~= 0));
%     ddTraj.minst.Optimization.CustomCostFcn = @(X,U,e,data,params) goal_weight * sum(X(end, 1:2) * normal_1' ./ (sqrt(sum(X(end, 1:2).^2, 2)) + 1e-8) .* exp(-sqrt(sum((X(end, 1:2) - obs_1_matrix).^2, 2)))) +...
%                                                                    goal_weight * sum(X(end, 1:2) * normal_2' ./ (sqrt(sum(X(end, 1:2).^2, 2)) + 1e-8) .* exp(-sqrt(sum((X(end, 1:2) - obs_2_matrix).^2, 2))));
...
function allevents_1(~,evt)
    ...
    switch(evname)
        case{'ROIMoved'}            
            ...
            axis equal;
            global obs_obj_1
            obs_obj_1 = evt.CurrentPosition;
            global obs_obj_2
            obs_radius = 0.2;
            rbt_radius = 0.18;
            rbt_pose = [0, 0, 0];

            draw_circle(obs_obj_1(1),obs_obj_1(2),obs_radius);
            draw_circle(obs_obj_2(1),obs_obj_2(2),obs_radius);
            draw_circle(rbt_pose(1),rbt_pose(2), rbt_radius);
