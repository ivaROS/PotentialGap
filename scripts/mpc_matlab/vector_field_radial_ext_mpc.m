...
% Plot robot triangle
rot_mat = [cos(rbt_pose(3)), -sin(rbt_pose(3)); sin(rbt_pose(3)), cos(rbt_pose(3))];
rbt_x = [rbt_pose(1)-0.1, rbt_pose(1)-0.1, rbt_pose(1)+0.5, rbt_pose(1)-0.1];
rbt_y = [rbt_pose(2)+0.3, rbt_pose(2)-0.3, rbt_pose(2)+0, rbt_pose(2)+0.3];
rbt = rot_mat * [rbt_x; rbt_y];
plot(rbt(1, :), rbt(2, :), 'r', 'LineWidth',2);
hold on;
...
plot([obs_obj_2(1), rbt_pose(1)], [obs_obj_2(2), rbt_pose(2)], 'r', 'LineWidth', 2);
hold on;
plot([obs_obj_1(1), rbt_pose(1)], [obs_obj_1(2), rbt_pose(2)], 'r', 'LineWidth', 2);

% radial extension
radial_ext = true;

if radial_ext
    gL = obs_obj_1(1:2) - rbt_pose(1:2);
    gR = obs_obj_2(1:2) - rbt_pose(1:2);
    dists = [norm(gL), norm(gR)];
    min_safe_dist = min(dists);
    eL = gL / norm(gL);
    eR = gR / norm(gR);
    eB = (eL + eR) / 2;
    eB = eB / norm(eB);
    ...
    qLp = gL - qB;
    qRp = gR - qB;
    pLp = [norm(qLp), atan2(qLp(2), qLp(1))];
    pRp = [norm(qRp), atan2(qRp(2), qRp(1))];
    phiB = pRp(2) - pLp(2);
    pB = [norm(-qB), atan2(-qB(2), -qB(1))];
    thL = pB(2) - gap_size / 4;
    thR = pB(2) + gap_size / 4;
    pLn = (thL - pLp(2)) / phiB * pRp + (pRp(2) - thL) / phiB * pLp;
    pRn = (thR - pLp(2)) / phiB * pRp + (pRp(2) - thR) / phiB * pLp;
    qLn = [pLn(1) * cos(pLn(2)), pLn(1) * sin(pLn(2))] + qB;
    qRn = [pRn(1) * cos(pRn(2)), pRn(1) * sin(pRn(2))] + qB;
    
    obs_obj_1(1:2) = qLn;
    obs_obj_2(1:2) = qRn;
    local_origin(1:2) = qB;

    draw_circle(qLn(1),qLn(2),obs_radius/2, 1, 'k');
    draw_circle(qRn(1),qRn(2),obs_radius/2, 1, 'k');

    hold on;
    plot([qLn(1), qB(1)], [qLn(2), qB(2)], 'm', 'LineWidth', 2);
    hold on;
    plot([qRn(1), qB(1)], [qRn(2), qB(2)], 'm', 'LineWidth', 2);
    axis off;
end
...
%         dev = goal_polar_derivative(0, coor);
        px(j,i) = dev(1);
        py(j,i) = dev(2);
    end
end

% Plot and field vectors
quiver(x, y, px, py, 'k');
hold on;
axis off;
...
function derivs = polar_derivative(t, y)
    ...
    global global_goal;
    global local_origin;
    obs1 = obs_obj_1 - local_origin;
    obs2 = obs_obj_2 - local_origin;
    theta_local = theta;
    goal_local = global_goal - local_origin;
    
    y_local = y' - local_origin(1:2);
    vec_1 = obs1(1:2) - y_local';
...
%     diff1 = angle1 - anglex;
    ...
    thetag = atan2(goal_local(2) - y_local(2), goal_local(1) - y_local(1));
    thetax = atan2(y_local(2), y_local(1));
    r1 = norm(obs1(1:2));
    r2 = norm(obs2(1:2));
    rx = norm(y_local);
...
function derivs = goal_polar_derivative(t, y)
...
function j = vector_heading_cost(input_x, use_goal_vec)
    ...
    for i = 1:size(input_x, 1)
        if use_goal_vec
            pg_vec = polar_derivative(0, input_x(i, 1:2)');
        else
...
function [t, w] = mpc_pg(times, y_init)
    ...
    vec_1 = obs1(1:2) - y_init(1:2);
    normal_1 = [1, -vec_1(1) / vec_1(2)];
    normal_1 = normal_1 / norm(normal_1);
    vec_2 = obs2(1:2) - y_init(1:2);
    normal_2 = [-1, vec_2(1) / vec_2(2)];
    normal_2 = normal_2 / norm(normal_2);
...
%                                                                        ];
    ddTraj.minst.Optimization.CustomIneqConFcn = @(X,U,e,data,params) [metaBuilder1.MV(1).min - U(1:end-1, 1); U(1:end-1, 1) - metaBuilder1.MV(1).max;...
                                                                       metaBuilder1.MV(2).min - U(1:end-1, 2); U(1:end-1, 2) - metaBuilder1.MV(2).max;...
                                                                       ];
