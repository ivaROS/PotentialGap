...
namespace potential_gap{
    ...
    struct Robot{
        ...
        bool holonomic = false, simple_boundary = true;
        ...
        Robot(RobotShape in_shape, double robot_length, double robot_width=0, double robot_avg_lin_speed=0.2, double robot_avg_rot_speed=0.5, 
            bool holonomic_mode=false, bool simple_boundary_flag=true)
        {
            shape = in_shape;
            holonomic = holonomic_mode;
            simple_boundary = simple_boundary_flag;
            switch (shape)
            {
    ...
    class RobotGeoProc{
        public:
            ...
            double getNearestDistance(Eigen::Vector2d& orientation_vec, Eigen::Vector2d& pt)
            {
                if(robot_.shape == RobotShape::circle)
                {
                    double near_dist = pt.norm();
                    return near_dist - robot_.radius;
                }
            ...
            double getRobotAvgRotSpeed()
            {
                return robot_.avg_rot_speed;
            }
